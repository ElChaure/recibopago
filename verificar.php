<?php
$username = $_GET['persona']; // get the username
$account =$_GET['ingreso'];
/*$dbconn = pg_connect("host=10.29.6.32 dbname=talento_humano user=robot password=#c@v3rn1c0l@")
        or die('Could not connect: ' . pg_last_error());*/
include ("consultar.php");
if(($_GET['persona']!='')&&($_GET['ingreso']=='')){

	echo check_username($username); 

}elseif (($_GET['persona']!='')&&($_GET['ingreso']!='')&&($_GET['persona']!='1')&&($_GET['ingreso']!='2')){

	echo check_username_account($username,$account); 

}elseif (($_GET['persona']!='')&&($_GET['ingreso']=='1')){

	echo obtener_datos($username,$account); 

}elseif (($_GET['persona']!='')&&($_GET['ingreso']=='2')){
...
function check_username($username){
	...
	if(pg_fetch_assoc($result)!=false){
		...
		return '<span style="color:#0c0">Cedula no Encontrada</span>';
	}
}
...
function check_username_account($username,$account){
	...
	if(pg_fetch_assoc($result)!=false){
		...
		return '<span style="color:#0c0">Fecha de Ingreso no coincide con registro</span>';
	}
}
